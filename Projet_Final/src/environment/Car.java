...
public class Car {
    ...
    private final Color colorSpec = Color.PINK;
     ...
     * @param : une case posG
     * @return : un boolean si la voiture et la grenouille sont sur la m�me case alors on renvois true,
     * false sinon
     */

    public boolean isAccident(Case posG) {
        // utiliser isSafe
        return (posG.ord == leftPosition.ord && (posG.absc >= this.leftPosition.absc)
                && (posG.absc < this.leftPosition.absc + this.length));
    }
    ...
    public void addSpecToGraphics(int ord) {
        game.getGraphic().add(new Element(leftPosition.absc, ord, colorSpec));
    }
