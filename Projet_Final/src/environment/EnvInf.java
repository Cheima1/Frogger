...
public class EnvInf implements IEnvironment {
    ...
    public void update() {
        for (Lane line : lanes) {
            ...
            if (idLaneSpecCar != -1) {
                if (!lanes.get(idLaneSpecCar).isSafe(game.getFrogPosition())) {
                    lanes.get(idLaneSpecCar).removeSpecialCase();
                    game.score += 1;
                    idLaneSpecCar = -1;
                }
            }
            else {
                idLaneSpecCar = (int)(Math.random() * lanes.size() - 1);
                lanes.get(idLaneSpecCar).addSpecialCase();
            }
